...
test_that('Check normal', {
  expect_error(rescale(x, type='n', mean='a'),
               'mean must be a numeric or integer value')
  expect_error(rescale(x, type='n', mean=0, sd='a'),
               'sd must be a positive numeric or integer value')
  expect_error(rescale(x, type='n', mean=0, sd=-1),
               'sd must be a positive numeric or integer value')
})

test_that('Check uniform', {
  expect_error(rescale(x, type='u', min='a'),
               'min must be a numeric or integer value')
  expect_error(rescale(x, type='u', min=0, max='a'),
               'max must be a numeric or integer value')
  expect_error(rescale(x, type='u', min=0, max=-1),
